...
import { handlePermissionCallback } from 'claude-to-im/src/lib/bridge/permission-broker.js';
import {
  buildCardContent,
  buildPostContent,
  hasComplexMarkdown,
...
import type { RuntimeName, SessionRecord } from '../runtime-types.js';
...
function previewKey(chatId: string, draftId: number): string {
...
export class FeishuAdapter extends BaseChannelAdapter {
  ...
  private activePreviewByChat = new Map<string, string>();
  private activeSessionFlows = new Map<string, SessionFlowState>();
  ...
  async stop(): Promise<void> {
    ...
    this.activePreviewByChat.clear();
    this.activeSessionFlows.clear();
  ...
  getPreviewCapabilities(chatId: string): PreviewCapabilities | null {
    ...
    if (!store.getChannelBinding(this.channelType, chatId)) {
      return null;
    }
    return {
      supported: true,
  ...
  async sendPreview(chatId: string, text: string, draftId: number): Promise<'sent' | 'skip' | 'degrade'> {
    ...
    const key = previewKey(chatId, draftId);
    let artifact = this.previewArtifacts.get(key);
    if (!artifact) {
      const createdArtifact = await this.createPreviewArtifact(chatId, draftId, processedText);
      if (!createdArtifact) return 'degrade';
      artifact = createdArtifact;
      this.previewArtifacts.set(key, artifact);
      this.activePreviewByChat.set(chatId, key);
    }
  ...
  endPreview(chatId: string, draftId: number): void {
    ...
    if (this.activePreviewByChat.get(chatId) === key) {
      this.activePreviewByChat.delete(chatId);
      this.activeSessionFlows.delete(chatId);
    }
  }
  ...
  async send(message: OutboundMessage): Promise<SendResult> {
    ...
    if (message.inlineButtons && message.inlineButtons.length > 0) {
      return this.sendPermissionCard(message.address.chatId, normalizeMarkdown(message), message.inlineButtons, message.replyToMessageId);
    }

    const activePreviewKey = this.activePreviewByChat.get(message.address.chatId);
    if (activePreviewKey) {
    ...
    const result = hasComplexMarkdown(text)
      ? await this.sendAsInteractiveCard(message.address.chatId, text, message.replyToMessageId)
      : await this.sendAsPost(message.address.chatId, text, message.replyToMessageId);
    if (result.ok) {
      void this.maybeRenameChat(message.address.chatId);
    }
    return result;
  ...
  private async handleIncomingEvent(data: FeishuMessageEventData): Promise<void> {
    ...
    this.lastIncomingMessageId.set(data.message.chat_id, messageId);
    const text = data.message.message_type === 'text' ? parseTextContent(data.message.content) : '';
    if (!text) return;

    await this.enqueueChatTask(data.message.chat_id, async () => {
  ...
  private async handleGroupMessage(_sender: SenderIdentity, inbound: InboundMessage): Promise<void> {
    ...
    if (lower.startsWith('/perm ')) {
      ...
      if (!handled) {
        await this.sendAsPost(inbound.address.chatId, '无效的权限命令，格式为 `/perm allow|allow_session|deny <id>`。', inbound.messageId);
      }
      return;
    }
    if (lower === '/reset') {
      await this.handleResetCommand(inbound.address.chatId, inbound.messageId);
      return;
    }
    if (lower.startsWith('/new')) {
      await this.sendAsPost(inbound.address.chatId, '请先私聊 Bot 使用 `/new:claude` 或 `/new:codex` 创建新会话。', inbound.messageId);
      return;
    }
    if (lower.startsWith('/')) {
      await this.sendAsPost(inbound.address.chatId, '该群仅支持普通对话、`/reset` 与 `/perm ...`。如需新会话，请私聊 Bot。', inbound.messageId);
      return;
    }

    const binding = this.getStore().getChannelBinding(this.channelType, inbound.address.chatId);
    if (!binding) {
      await this.sendAsPost(inbound.address.chatId, '当前群尚未绑定会话。请先私聊 Bot 发送 `/new:claude` 或 `/new:codex`。', inbound.messageId);
      return;
    }

    const ext = this.getStore().getSessionExt(binding.codepilotSessionId);
    if (ext?.runtime) {
      this.activeSessionFlows.set(inbound.address.chatId, {
  ...
  private async handleCreateSessionCommand(sender: SenderIdentity, inbound: InboundMessage, runtime: RuntimeName): Promise<void> {
    ...
    try {
      ...
      this.activeSessionFlows.set(chatId, { sessionId: session.id, runtime });
      await this.sendAsPost(chatId, `已创建 ${runtime} 会话。后续请直接在本群继续对话。`);
      await this.sendAsPost(inbound.address.chatId, `已创建新群并绑定 ${runtime} 会话。`, inbound.messageId);
    } catch (error) {
      ...
      if (chatId) {
        await this.sendAsPost(chatId, `初始化失败：${message}`);
      }
      await this.sendAsPost(inbound.address.chatId, message, inbound.messageId);
    }
  }

  private async handleResetCommand(chatId: string, replyToMessageId?: string): Promise<void> {
    const store = this.getStore();
    const binding = store.getChannelBinding(this.channelType, chatId);
    if (!binding) {
      await this.sendAsPost(chatId, '当前群尚未绑定会话，请先私聊 Bot 使用 `/new:claude` 或 `/new:codex`。', replyToMessageId);
      return;
    }
    ...
    const updated = store.getChannelBinding(this.channelType, chatId);
    if (updated) {
      store.updateChannelBinding(updated.id, { mode: binding.mode, sdkSessionId: '' });
    }
    this.activeSessionFlows.set(chatId, { sessionId: session.id, runtime });
    await this.sendAsPost(chatId, `已重置当前群会话，runtime 保持为 ${runtime}。`, replyToMessageId);
  }
  ...
  private async createPreviewArtifact(chatId: string, draftId: number, text: string): Promise<PreviewArtifact | null> {
    ...
    try {
      ...
      const sendResponse = replyToMessageId
      ...
      try {
        const sendResult = await this.sendInteractiveCard(chatId, buildSimpleCard(text), replyToMessageId);
        return {
          key: previewKey(chatId, draftId),
          chatId,
          draftId,
  ...
  private async sendPermissionCard(
    ...
    const result = await this.sendInteractiveCard(chatId, buildPermissionCard(text, buttons), replyToMessageId);
    return {
      ok: true,
  ...
  private async sendAsInteractiveCard(chatId: string, text: string, replyToMessageId?: string): Promise<SendResult> {
